...
// フォーム送信後にPlain chatへ内容を連携
(function () {
  // ---- 1. Plainウィジェット読み込み（バブル非表示）----
  ...
  var css = [
    ...
    '#pf-sent svg{margin-bottom:12px}',
  ...
  document.body.insertAdjacentHTML('beforeend', [
    ...
    '<div id="pf-overlay">',
    '  <div id="pf-modal">',
    '    <button id="pf-close" aria-label="閉じる">&times;</button>',
    '    <div id="pf-form-wrap">',
    '      <h2>お問い合わせ</h2>',
    ...
    '    </div>',
    '    <div id="pf-sent">',
    '      <svg width="48" height="48" viewBox="0 0 24 24" fill="none" stroke="#6E42EC" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"/><polyline points="16 12 12 16 8 12"/><line x1="12" y1="8" x2="12" y2="16"/></svg>',
    '      <h3>送信が完了しました！</h3>',
  ...
  form.addEventListener('submit', function (e) {
    ...
    try {
      Plain.prefill({ message: msg.join(' ') });
    } catch (err) {}

    try {
      Plain.open();
      // Plainが開いたら最初のメッセージとして送信
      setTimeout(function () {
        try { Plain.sendMessage(msg.join(' ')); } catch (e2) {}
      }, 800);
    } catch (err) {}
