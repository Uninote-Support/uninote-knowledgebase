---
title: Create an Organization
---

<Note>This document explains the steps to create a workspace (organization) after creating a Uninote account.</Note>


***

### Steps

#### 1. Create an Account

1. Access the Uninote sign-up page and click on [Sign Up].\
   ![](</images/image (2).png>)
2. Choose your registration method.\
   ![](</images/image (1) (1) (1) (1).png>)

* If registering with an email address:
  1. Enter your name, email address, and password, then click [Register].\
     ![](</images/image (2) (1).png>)
  2. Click the link in the verification email sent to your registered email address.

      📧 Subject: `Verify your Uninote account` Sender: `no-reply@mail.uninote.ai`
* If registering with a Google / Microsoft account:
  1. Select "Sign in with Google" or "Sign in with Microsoft".\
     ![](</images/image (3).png>)
  2. Choose the account you want to use and grant permission.

#### 2. Create an Organization

1. After logging in, the workspace creation screen will be displayed.\
   ![](</images/image (4).png>)
2. Fill in the required fields and click [Get Started].

***

### Next Steps

Once the creation is complete, proceed to invite users.

→ 1️⃣ [Add Users (Invite)](yz-1.md)
